    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        binding.button5.setOnClickListener {
            if (binding.name.text.isEmpty())
                Toast.makeText(requireContext(),"Enter Your Buddies Name",Toast.LENGTH_SHORT).show()
            if (binding.autoCompleteTxt.text.isEmpty())
                Toast.makeText(requireContext(),"Selet The type of Pet",Toast.LENGTH_SHORT).show()
            if (binding.age.text.isEmpty())
                Toast.makeText(requireContext(),"Enter Your Buddies Age",Toast.LENGTH_SHORT).show()
            if (binding.breed.text.isEmpty())
                Toast.makeText(requireContext(),"Enter Your Pets Breed",Toast.LENGTH_SHORT).show()
            ...
            if (binding.petHealth.text.isEmpty())
                Toast.makeText(requireContext(),"Give a health diagnosis",Toast.LENGTH_SHORT).show()
            if (binding.adoptionDays.text.isEmpty())
                Toast.makeText(requireContext(),"Enter the number of days you can keep yr pet for",Toast.LENGTH_SHORT).show()
            if (binding.petAdress.text.isEmpty())
                Toast.makeText(requireContext(),"Enter Your Buddies Address",Toast.LENGTH_SHORT).show()
            if (binding.message.text.isEmpty())
                Toast.makeText(requireContext(),"Enter description of your Buddy",Toast.LENGTH_SHORT).show()
            if(!(binding.radioButtonMale.isChecked) && !(binding.radioButtonFemale.isChecked))
                Toast.makeText(requireContext(),"Enter Gender of your Buddy",Toast.LENGTH_SHORT).show()
...
// Check if the strings are not empty before converting to numeric types
        ...
        val adoptionDays = if (adoptionDaysText.isNotEmpty()) adoptionDaysText.toInt() else 0
    ...
    fun uploadImageToCloudinaryb(selectedImageUri: Uri) {
